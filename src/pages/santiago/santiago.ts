              ...
              for (var menu of menus){
                ...
                if (attributes.canteen == "Refeitório de Santiago"){
                  ...
                  if (attributes.disabled != "0") {
                    this.santiagoMeals.push({header: date + ", " + attributes.meal , content: [attributes.disabled]})
                  } else {
                    var arr = [];
                    for (var item of items.item){
                      arr.push(item);
                    }
                    this.santiagoMeals.push({header: date + ", " + attributes.meal , content: arr});
                  }
                }
              }
              this.storage.set('menu',this.santiagoMeals);
            } else {
