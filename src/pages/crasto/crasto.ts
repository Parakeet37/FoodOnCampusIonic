              ...
              for (var menu of menus){
                ...
                if (attributes.canteen == "Refeitório do Crasto"){
                  ...
                  if (attributes.disabled != "0") {
                    this.crastoMeals.push({header: date + ", " + attributes.meal , content: [attributes.disabled]})
                  } else {
                    var arr = [];
                    for (var item of items.item){
                      arr.push(item);
                    }
                    this.crastoMeals.push({header: date + ", " + attributes.meal , content: arr});
                  }
                }
              }
              this.storage.set('menu',this.crastoMeals);
            } else {
